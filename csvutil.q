...
info0:{[file;onlycols]
  ...
  info:update t:"G",(rules:rules,'52) from info where t="*",mw=36,mdot=0,{all x like"????????-????-????-????-????????????"}peach sdv,.csv.cancast["G"]peach sdv; / GUID, v3.0 or later
  info:update t:"N",(rules:rules,'53),maybe:1b from info where t="n",mw=15,mdot=0,{all x in"0123456789"}each dchar,.csv.cancast["N"]peach sdv; / N, could be T but that'd loose precision
  info:update t:"T",(rules:rules,'54),maybe:1b from info where t="n",mw=9,mdot=0,{all x in"0123456789"}each dchar,.csv.cancast["T"]peach sdv;
  info:update t:"G",(rules:rules,'55) from info where t="*",mw=38,mdot=0,{all x like"{????????-????-????-????-????????????}"}peach sdv,.csv.cancast["G"]peach sdv; / GUID, v3.0 or later
